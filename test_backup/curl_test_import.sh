...
LOGIN_RESPONSE=$(curl -s -b "$COOKIE_JAR" -c "$COOKIE_JAR" -X POST     -H "Content-Type: application/x-www-form-urlencoded"     -d "emailOrUsername=admin@example.com&password=Admin123!"     http://localhost:3000/auth/login)
...
if echo "$IMPORT_PAGE" | grep -q "Access Denied"; then
    ...
    if echo "$IMPORT_PAGE" | grep -q 'input.*type="file".*name="puzzleFile"'; then
